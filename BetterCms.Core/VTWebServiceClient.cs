...
namespace BetterCms.Core.WebServices
{
    ...
    {
        #region Fields
        //private static string _baseApiPath = ConfigurationManager.AppSettings["ApiPath"]; // "http://localhost:42705/api/";//ConfigurationManager.AppSettings["ApiPath"];
        private static string _baseApiPath = ConfigurationManager.ConnectionStrings["ApiPath"].ConnectionString; // "http://localhost:42705/api/";//ConfigurationManager.AppSettings["ApiPath"];
        ...
        public T UploadData<T>(string resourceName, object postData)
        {
            try
            {
                T t = default(T);
                ...
                if (postData != null && !String.IsNullOrEmpty(resourceName.Trim()))
                {
                    ...
                    // _webClient.Headers.Add(HttpRequestHeader.Authorization, "amx " + GetHashCode(Encoding.UTF8.GetBytes(data), address, "POST"));
        ...
        private string GetHashCode(byte[] content, string url, string requestHttpMethod)
        {
            string APPId = _APPId;
    ...
    //       var model = _webClient.DownloadData<string>("Values/addnewrole", new { name = name, description = description });
}
