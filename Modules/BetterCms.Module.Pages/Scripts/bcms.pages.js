        ...
        */
        page.initializeSiteSettingsPagesList = function (content, jsonData, opts, isSearchResult) {
            ...
            try {
                $.each(ids.split(','), function (index, value) {
                    $('div[data-id=' + value + '].bcms-grid-item-edit-button').hide();
                    $('div[data-id=' + value + '].bcms-grid-item-delete-button').hide();
                });
            }
            catch (e) { }
        ...
        function PageItemViewModel(item, parent, canBeSelected) {
            ...
            if (typeof item.PageId == 'undefined')
                ...
                self.id = ko.observable(item.PageId);
        ...
        */
        page.addSiteSettingsPage = function (container, opts) {
            page.openCreatePageDialog(function (json) {
                if (json.Data != null) {
                    page.pagesGridViewModel.items.unshift(new PageItemViewModel(json.Data, null, true));
                }
            });
        };
