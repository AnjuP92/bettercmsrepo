...
namespace BetterCms.Module.Root.Services
{
    public class DefaultLanguageService : ILanguageService
    {
        ...
        {
            IEnumerable<Language> Ilanguagelist = new List<Language>();
            List<Language> languagelist = new List<Language>();
            var model = _webClient.DownloadData<JObject>("Root/ApiFetchContent", new { });
            for (int i = 0; i < model.Count; i++)
            {
                Language languageobject = new Language();
                languageobject.Name = model[i]["Name"].ToString();
                languageobject.Code = model[i]["Code"].ToString();
                languagelist.Add(languageobject);
            }
            Ilanguagelist = languagelist;
