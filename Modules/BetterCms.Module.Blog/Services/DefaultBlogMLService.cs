    ...
    public class DefaultBlogMLService : IBlogMLService
    {
        ...
        private IDictionary<string, Guid> ImportAuthors(BlogMLBlog.AuthorCollection authors, IList<Author> createdAuthors, List<BlogMLPost> blogs)
        {
            ...
            if (authors != null)
            {
                foreach (var authorML in authors)
                {
                    ...
                    //var response = _webClient.DownloadData<string>("Blog/GetAuthorId", new { title = authorML.Title });
                    //var id = new Guid(response);
                    var id = repository
                        .AsQueryable<Author>()
                    ...
                    if (id.HasDefaultValue())
                    {
                        ...
                        //authordetails.Add("name", author.Name);
                        //if(author.Image != null)
                        //{
                        //    authordetails.Add("imageId", author.Image.Id);
                        //}
                        //else
                        //{
                        //    authordetails.Add("imageId", null);
                        //}

                        //authordetails.Add("description",author.Description);
                                                           ...
                                                           bool RecreateCategoryTree)
        {
            ...
            if (RecreateCategoryTree)
            {
                ...
                var existingcategoriesIdsModel = _webClient.DownloadData<JArray>("Blog/GetBlogExistingCategoryIds", new { });
                List<Guid> categorylist = new List<Guid>();
                for (int i = 0; i < existingcategoriesIdsModel.Count; i++)
                {

                    categorylist.Add(new Guid(existingcategoriesIdsModel[i]["Id"].ToString()));
                }

                //var existingCategoriesIds = new List<Category>();
                //existingCategoriesIds = categorylist;
                List<Guid> existingcategorylist = new List<Guid>();
                foreach (var category in oldIdsCategories)
                {
                    foreach (var existingcategory in categorylist)
                    {
                        if (category == existingcategory)
                        {
                            existingcategorylist.Add(existingcategory);
                        }
                    }
                }
                var existingCategoriesIds = existingcategorylist;
                ...
                var categorizableItemModel = _webClient.DownloadData<JArray>("Blog/GetCategorizableItem", new { });

                List<CategorizableItem> categorizableItemslist = new List<CategorizableItem>();

                for (int i = 0; i < categorizableItemModel.Count; i++)
                {
                    CategorizableItem Item = new CategorizableItem();
                    Item.Id = new Guid(categorizableItemModel[i]["ItemId"].ToString());
                    Item.Version = Convert.ToInt32(categorizableItemModel[i]["Version"]); ;
                    Item.IsDeleted = (bool)categorizableItemModel[i]["IsDeleted"];
                    Item.CreatedOn = (DateTime)categorizableItemModel[i]["CreatedOn"];
                    Item.CreatedByUser = categorizableItemModel[i]["CreatedByUser"].ToString();
                    Item.ModifiedOn = (DateTime)categorizableItemModel[i]["ModifiedOn"];
                    Item.ModifiedByUser = categorizableItemModel[i]["ModifiedByUser"].ToString();
                    if (!string.IsNullOrEmpty(categorizableItemModel[i]["DeletedOn"].ToString()))
                    {
                        Item.DeletedOn = (DateTime)categorizableItemModel[i]["DeletedOn"];
                    }
                    else
                    {
                        Item.DeletedOn = null;
                    }
                    Item.DeletedByUser = (!string.IsNullOrEmpty(categorizableItemModel[i]["DeletedByUser"].ToString())) ? categorizableItemModel[i]["DeletedByUser"].ToString() : null;
                    Item.Name = categorizableItemModel[i]["Name"].ToString();
                    categorizableItemslist.Add(Item);

                }

                var availableFor = new List<CategorizableItem>();
                availableFor = categorizableItemslist;
