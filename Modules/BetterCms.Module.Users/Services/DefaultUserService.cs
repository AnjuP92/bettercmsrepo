...
namespace BetterCms.Module.Users.Services
{
    public class DefaultUserService : IUserService
    {
        ...
        {
            return _webClient.DownloadData<string>("User/ApiGetUserNameByEmail", new { Email = email });
        }
        ...
        public User SaveUser(EditUserViewModel model, bool treatNullsAsLists = true, bool createIfNotExists = false)
        {
            ...
            if (!isNew)
            {
                ...
                //JObject response = _webClient.DownloadData<JObject>("User/ApiGetUserInfo", new { UserId = model.Id });


                //var userDetails = new { Id = response.GetValue("Id").Value<string>(), UserName = response.SelectToken("UserName").Value<string>(), FirstName = response.SelectToken("FirstName").Value<string>(), LastName = response.SelectToken("LastName").Value<string>(), Password = response.GetValue("Password").Value<string>(), Salt = response.GetValue("Salt").Value<string>(), ImageId = response.GetValue("ImageId").Value<string>() };
                //user.Id = new Guid(userDetails.Id);
                //user.UserName = userDetails.UserName;
                //user.FirstName = userDetails.FirstName;
                //user.LastName = userDetails.LastName;
                //user.Password = userDetails.Password;
                //user.Salt = userDetails.Salt;
                //if (userDetails.ImageId != null)
                //{
                //    user.Image.Id = new Guid(userDetails.ImageId);
                //}
                ...
                ////         userRole.Role.Name   = item.ToString();
            ...
            if (model.Image != null && model.Image.ImageId.HasValue)
            {
                ...
                //JObject response = _webClient.DownloadData<JObject>("User/ApiGetImageInfo", new { ImageId = model.Image.ImageId });

                //user.Image = new MediaImage();
                //var userDetails = new { Id = response.GetValue("Id").Value<string>(), Title = response.SelectToken("Title").Value<string>() };
                //Guid newid = new Guid(userDetails.Id);
                //user.Image.Id = newid;
                //user.Image.Title = userDetails.Title;
            ...
            //    if (user.Image == null)
            //    {
            //        usermodel = _webClient.DownloadData<string>("User/ApiSaveUserwithoutimage", new { id = user.Id, version = user.Version, UserName = user.UserName, FirstName = user.FirstName, LastName = user.LastName, Email = user.Email, Password = user.Password, Salt = user.Salt });
            //    }
            //    else
            //    {

            //        usermodel = _webClient.DownloadData<string>("User/ApiSaveUserwithImage", new { id = user.Id, version = user.Version, UserName = user.UserName, FirstName = user.FirstName, LastName = user.LastName, Email = user.Email, Password = user.Password, ImageId = user.Image.Id.ToString(), Salt = user.Salt });
            //    }


            //    Guid generatedId = new Guid(usermodel);
            //    user.Id = generatedId;
            //}
            ...
            //    if (user.Image == null)
            //    {
            //        usermodel = _webClient.DownloadData<string>("User/ApiUpdateUserwithoutImage", new { id = user.Id, version = user.Version, UserName = user.UserName, FirstName = user.FirstName, LastName = user.LastName, Email = user.Email });
            //    }
            //    else
            //    {

            //        usermodel = _webClient.DownloadData<string>("User/ApiUpdateUserwithImage", new { id = user.Id, version = user.Version, UserName = user.UserName, FirstName = user.FirstName, LastName = user.LastName, Email = user.Email, ImageId = user.Image.Id.ToString() });
            //    }
        ...
        /// Saved user entity
        /// </returns>
        public User SaveSocialUser(EditUserViewModel model, bool treatNullsAsLists = true, bool createIfNotExists = false)
        {
            ValidateUser(model);
        ...
        private List<Role> SaveUserRoles(User user, EditUserViewModel request, bool createIfNotExists)
        {
            ...
            var rolesToInsert = requestRoles
                ...
                .ToList();
            //var rolesToInsert = request.Roles;
            ...
            if (rolesToInsert.Count > 0)
            {
                ...
                //foreach (var roleitem in rolesToInsert)
                //{
                //    string roleidfromdb = _webClient.DownloadData<string>("User/ApiGetRoleId", new { RoleName = roleitem });
                //    Guid roleid = Guid.Parse(roleidfromdb);
                //    if (roleid != null)
                //    {
                //        var roleInserted = _webClient.DownloadData<int>("User/ApiAddUserRole", new { UserId = user.Id, RoleId = roleid });
                //    }
                //}
        ...
        private void ValidateUser(EditUserViewModel model)
        {
            ...
            //    .Select(r => r.Id)
            //    .FirstOrDefault();
            var usermodel = _webClient.DownloadData<int>("User/ApiValidateUserName", new { Id = model.Id, UserName = userName });

            //var usermodel=ApiValidateUserName(model.Id,)
            ...
            usermodel = _webClient.DownloadData<int>("User/ApiValidateUserEmail", new { Id = model.Id, Email = email });
        ...
        public void DeleteUser(Guid id, int version)
        {
            ...
            JObject response = _webClient.DownloadData<JObject>("User/ApiGetUserInfo", new { UserId = id });
            //Guid IdFromJObj = new Guid(response.GetValue("Id").Value<string>());
            //user.Id = new Guid(response.GetValue("Id").Value<string>());
            ...
            var model = _webClient.DownloadData<int>("User/DeleteUser", new { UserId = id });
            ...
            var count = _webClient.DownloadData<int>("User/DeleteRole", new { UserId = id });
